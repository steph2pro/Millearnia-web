...
import useInterestDelete from '../hook/useInterestDelete';
...
const InterestDelete = () => {
  ...
  const navigate = useNavigate();
      const { id } = useParams();
  const { register, handleSubmit, onSubmit, isDeleted } = useInterestDelete( parseInt(id) );
      ...
      <div className="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            ...
            <div className="sm:flex sm:items-start">
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                <h3 className="text-lg font-medium leading-6 text-gray-900" id="modal-title">
                  Supprimer le centre d'interet
                </h3>
                <div className="mt-2">
                  <p className="text-sm text-gray-500">
                    Êtes-vous sûr de vouloir supprimer ce centre d'interet ?
                  </p>
                </div>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
            ...
            <button
                onClick={()=>navigate(STRING_ROUTE_INTERESTS)} type="button" className="inline-flex justify-center w-full px-4 py-2 mt-3 text-base font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" data-bs-dismiss="modal">
              Annuler
            </button>
          </form>
        </div>
