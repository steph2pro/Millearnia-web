...
import useDeleteProfession from '../hook/useDeleteProfession';
import { useNavigate, useParams } from 'react-router-dom';
import useVideoDelete from '../hook/useVideoDelete';

const VideoDelete = () => {
  const navigate = useNavigate();
    
      const { id } = useParams();
  const { register, handleSubmit, onSubmit, isDeleted } = useVideoDelete( parseInt(id) );
      ...
      <div className="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
            ...
            <button 
              onClick={()=>navigate(STRING_ROUTE_VIDEOS)} type="button" className="inline-flex justify-center w-full px-4 py-2 mt-3 text-base font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" data-bs-dismiss="modal">
              Annuler
            </button>
          </form>
        </div>
