...
import useCategoryDelete from '../hook/useCategoryDelete';
...
const CategoryDelete = () => {
  ...
  const navigate = useNavigate();
      const { id } = useParams();
  const { register, handleSubmit, onSubmit, isDeleted } = useCategoryDelete( parseInt(id) );
      ...
      <div className="flex items-end justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div className="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              ...
              <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                ...
                <div className="mt-2">
                  <p className="text-sm text-gray-500">
                    Êtes-vous sûr de vouloir supprimer cette Category ?
                  </p>
                </div>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="px-4 py-3 bg-gray-50 sm:px-6 sm:flex sm:flex-row-reverse">
            ...
            <button
                onClick={()=>navigate(STRING_ROUTE_CATEGORIES)} type="button" className="inline-flex justify-center w-full px-4 py-2 mt-3 text-base font-medium text-gray-700 bg-white border border-gray-300 rounded-md shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm" data-bs-dismiss="modal">
              Annuler
            </button>
          </form>
        </div>
